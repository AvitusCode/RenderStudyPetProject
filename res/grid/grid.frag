...
vec4 grid(vec3 fragPos3D, float scale)
{
	...
	// X axis
	if (fragPos3D.z > -0.5 * minimumx && fragPos3D.z < 0.5 * minimumx){
		color.x = 1.0;
	}
...
float computeLinearDepth(vec3 pos)
{
	...
	float near = (2.0 * b) / (2.0 * a - 2.0);
	float far = ((a - 1.0) * near) / (a + 1.0);

	vec4 clip_space_pos = Projection * View * vec4(pos, 1.0);
	float ndc = (clip_space_pos.z / clip_space_pos.w) * 2.0 - 1.0; // Put back between -1;1
...
{	
	...
	FragColor = (grid(fragPos3D, 1) + grid(fragPos3D, 0.1)) * float(t > 0);
	FragColor.a *= fading;
}